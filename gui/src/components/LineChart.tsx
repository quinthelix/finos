...
function scaleMultipleSeries(
  ...
  const minX = opts?.xDomain ? opts.xDomain.min.getTime() : Math.min(...xs)
  const maxX = opts?.xDomain ? opts.xDomain.max.getTime() : Math.max(...xs)
          ...
          {!isAnimating && lineSeries.map(s => 
            (isOverlayMode ? s.scaledPoints.filter((_, i, arr) => i === 0 || i === arr.length - 1) : s.scaledPoints).map((p, i) => (
              <circle
                ...
                data-kind="dot"
                data-x={p.originalX.getTime()}
                data-stroke={(p.strokeColor || s.color) ?? 'none'}
                data-quality={p.strokeColor ? '1' : undefined}
                cx={p.x}
                cy={p.y}
                r={3}
                fill="#0a0e17"
