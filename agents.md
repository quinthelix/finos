...
- **Containers** for each service, build docker to github regustry; deployable to **Fly.io**.
...
- Auth handled via **Supabase Auth**, enforced primarily at the **gateway** and propagated to services.
...
│   │                      #configurable on which 3rd party to use. can be mulitple 3rd parties. 
...
│   ├─ risk-engine/
│   │    
│   ├─ trade-gateway/        # provides the full api to trade and selects internaly 
│   │  │                     # which broker to use. 
...
- **follow the folder structure**. if you feel more folder are needed ask for approval and update this file. 
...
- must use uv tool as packge, builder and runner
- must build to a dockerized application. 
- log using **structlog**

## 3.2 Front end
the front code is developed in gui folder under the root. it must also have an agents.md file describing its
purpose and design. 
...
## 3.4 Databases 
we will use postgress standard as a relational database. in development env the container will be run. 
in production env we will connect to the superbase account. 
...
- **Frontend–backend:** REST/JSON to the **gateway**.
- **ERP-sim:** is treated as an **external system**:
  - Only `erp-extractor` talks to it (no GUI direct access).
- **Broker sim** is access via provider in trade-gateway
...
- 1️⃣ All global rules live here at root; per-service `agents.md` files must not contradict this.
- 2️⃣ `erp-sim` simulates the customer ERP; it owns tables and its own API, and is **not behind the gateway**.
- 3️⃣ Trading is done via a **trade-service** with a pluggable provider abstraction, including a **broker-sim** provider that mimics a real trading API.
- 4️⃣ Auth is handled via Supabase Auth (Phase 1), with the gateway enforcing access control and propagating `company_id`/roles to internal services.
...
- **API** enables to get commodoty price from db based on name, id and timestamp. 
...
- **Data Ownershio** Owns “raw ERP” style tables:
  - Purchase orders, inventory, forecast, maybe BOM.
- **API** Exposes **REST API** only to erp-extractor using REST. This API is **NOT behind the gateway**. e.g: 
...
- **Data ownership** writes customer specific ERP extracted data, in records to customer_id raw_erp in postgress. 
- **API** enables reading the custoemr specific EPR data from the postgress DB. 
...
- **Purpose:** Calculates and stores various risk exposures to the company based on its purchasing plan form the erp. 
- **Data ownership** writes customer risk calculations to table `exposures` in the database. 
- **API** enables calculatig and reading the customer exposures based on purchase plan, market prices and forecasts.
- **Used By** `gui` (via `api-gateway`) to trigger risk calculations, retreive the reuslts in order to present them visually 

### 4.5 `trade-service` (centerlized trade api encapsulating different providers)
...
- **Data ownership** writes `transations` and `positions` tables in database for history analysis
- **API** perform trade on specific commodities financial tools such as futures and options. 
- **Used By** `gui` (via `api-gateway`) initiate a trade operation via one of the providers, or to get historic trade actions in order to present it visually 

### 4.6 `providers` (trade broker implementation)
- **Purpose:** specific broker connection implementation. the `trade-service` uses it to preform actions against an external broker. each 3rd party broker should have an implemention of a provider
- **Data ownership** TBD
- **API** TBD to decide on common API for this layer.
- **Used By** `trade-service` only, in order to initiate trade using a specific broker, and get its results. 

### 4.6 `api-gateway` (API gateway / backend-for-frontend)
...
- Exposes **REST/JSON API**:
  ...
  - `POST /api/trades`
- Responsibilities:
  - Phase 0: planly enter company_id, user_id from caller for demo purposes. 
...
**Rule:** GUI should only talk to the **api-gateway**, not to other services directly.

### 4.7 `gui` (frontend)

- **Purpose:** User-facing interface for the platform. shows the user the company summary, deep dive on commodities, exposures, positioins and trade actions. 
- **Views** to be detailed in folder specific agents.md file. 
  
## 5. Environments & Configuration

the code base builds in support for 

- **Local development** uses docker-compose and local container registry, persistent volumes for db. 
- **Remote PoC** (Fly.io + Supabase) uses github container registry, superbase for persistent data. 
...
| `GATEWAY_PUBLIC_URL`       | Public URL for gateway                         | `http://localhost:8080`                    | `https://<fly-app-name>.fly.dev`               |
...
| `JWT_JWKS_URL`             | URL to fetch JWKS for JWT verification (later) | *(placeholder)*                            | From Supabase project (for gateway)            |
...
- Fly.io hosts:
  ...
  - Gateway as the only public HTTP endpoint for the platform.
...
- Internal services trust `company_id` input from gateway.

### Phase 1 – Supabase Auth + JWT verification at Gateway
...
- Gateway:
  ...
  - Maps user → `company_id` (via DB table).
...
7. Implement trading integrations via `trade-service` providers; do not call brokers from gateway or GUI.
